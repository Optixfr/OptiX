import { inject, injectAsync } from '@angular/core';
...
export const EyesMeasureStore = signalStore(
  ...
  withMethods((
    store,
    // Lazy-load the calculation service (and its chunk) only when a report is
    // actually generated, via Angular 22's `injectAsync()`.
    loadCalculationService = injectAsync(() =>
      import('../services/calculation/eyes-calculation.service').then(
        (m) => m.EyesCalculationService
      )
    ),
